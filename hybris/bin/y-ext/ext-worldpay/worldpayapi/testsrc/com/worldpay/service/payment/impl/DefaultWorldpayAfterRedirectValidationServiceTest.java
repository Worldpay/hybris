...
public class DefaultWorldpayAfterRedirectValidationServiceTest {
    ...
    private Map<String, String> createFullWorldpayResponse() {
        ...
        response.put(PAYMENT_STATUS, AUTHORISED.name());
        response.put(ORDER_KEY, ORDER_KEY);
        response.put(KEY_MAC, KEY_MAC);
        response.put(KEY_PAYMENT_AMOUNT, String.valueOf(PAYMENT_AMOUNT));
        response.put(KEY_PAYMENT_CURRENCY, GBP);
    ...
    private Map<String, String> createFullWorldpayResponseWithMac2() {
        ...
        response.put(PAYMENT_STATUS, AUTHORISED.name());
        response.put(ORDER_KEY, ORDER_KEY);
        response.put(KEY_MAC2, KEY_MAC2);
        response.put(KEY_PAYMENT_AMOUNT, String.valueOf(PAYMENT_AMOUNT));
        response.put(KEY_PAYMENT_CURRENCY, GBP);
