...
export class WorldpayB2BApmSepaComponent extends WorldpayApmSepaComponent implements OnInit {
  ...
  protected addSaveFieldToForm(): void {
    this.worldpayApmFacade.getSaveApm().pipe(
      tap((save: boolean): void => {
        if (this.sepaForm.contains('save')) {
          this.sepaForm.get('save')?.setValue(save ?? false);
          return;
        }

        this.sepaForm.addControl('save', this.fb.control(save ?? false));
      }),
  ...
  protected override beforeCreatePaymentDetails(details: ApmPaymentDetails): ApmPaymentDetails {
    this.worldpayApmFacade.setSaveApm(this.sepaForm.get('save').value);
    return {
      ...details,
      ...this.sepaForm.value
    };
