...
const mockReplenishmentOrderFormData$ = new BehaviorSubject<ScheduleReplenishmentForm>(mockReplenishmentOrderFormData);

class MockCheckoutReplenishmentFormService implements Partial<CheckoutReplenishmentFormService> {
  getOrderType = createSpy().and.returnValue(of(ORDER_TYPE.SCHEDULE_REPLENISHMENT_ORDER));
  getScheduleReplenishmentFormData = createSpy().and.returnValue(mockReplenishmentOrderFormData$.asObservable());
...
describe('WorldpayCheckoutScheduledReplenishmentPlaceOrderComponent', () => {
  let component: WorldpayCheckoutScheduledReplenishmentPlaceOrderComponent;
  let fixture: ComponentFixture<WorldpayCheckoutScheduledReplenishmentPlaceOrderComponent>;
  ...
  beforeEach(() => {
    fixture = TestBed.createComponent(WorldpayCheckoutScheduledReplenishmentPlaceOrderComponent);
  ...
  it('should call place ACH Order when ACHForm is valid and ORDER_TYPE.PLACE_ORDER', () => {
    component.ngOnInit();
    fixture.detectChanges();
  ...
  describe('when order type is PLACE_ORDER', () => {
    ...
    it('should place order when checkbox checked', () => {
      ...
      expect(component.placeWorldpayOrder).toHaveBeenCalled();
      expect(component.executeDDC3dsJwtCommand).toHaveBeenCalled();
      expect(component.scheduleReplenishmentFormData).toBeUndefined();
      component['processDdc']({
        origin: 'https://centinelapistag.cardinalcommerce.com',
        data: JSON.stringify({
          SessionId: '12345',
          Status: true
  ...
  describe('when order type is SCHEDULE_REPLENISHMENT_ORDER', () => {
    ...
    it('should schedule a replenishment order when checkbox checked', () => {
      spyOn(orderFacade, 'startLoading');
      spyOn(scheduledReplenishmentOrderFacade, 'scheduleReplenishmentOrder').and.returnValue(of({}));
      spyOn(orderFacade, 'initialPaymentRequest').and.returnValue(of({ order: { code: '00001' } }));
      // @ts-ignore
      spyOn(component, 'ddcIframeHandler').and.callThrough();
      // @ts-ignore
      ...
      expect(orderFacade.startLoading).toHaveBeenCalledWith(component['vcr']);
      expect(globalMessageService.add).toHaveBeenCalledWith({ key: 'checkoutReview.placingOrder' }, GlobalMessageType.MSG_TYPE_INFO);
      expect(component.scheduleReplenishmentFormData).toEqual(mockReplenishmentOrderFormData);
      expect(scheduledReplenishmentOrderFacade.scheduleReplenishmentOrder).toHaveBeenCalledWith(
        component.scheduleReplenishmentFormData,
        true
      );
    });
