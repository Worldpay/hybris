...
import { WorldpayApmFacade, WorldpayCheckoutPaymentFacade, WorldpayOrderFacade } from '../../facade';
...
export class WorldpayApmService implements WorldpayApmFacade {
  ...
  constructor(
    ...
    this.setWorldpaySavedCreditCardEvent();
    this.setWorldpaySaveAsDefaultCreditCardEvent();
   ...
   * @since 6.4.0
   */
  getAPMRedirectUrl(apm: ApmData): void {
    this.getWorldpayAPMRedirectUrl(apm, this.save$.value).pipe(
      take(1)
    ).subscribe({
