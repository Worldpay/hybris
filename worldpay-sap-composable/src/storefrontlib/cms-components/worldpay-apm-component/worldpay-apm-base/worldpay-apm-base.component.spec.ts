...
describe('WorldpayApmBaseComponent', () => {
  ...
  describe('disableContinueButton$', () => {
    ...
    it('should use cached billingAddressForm status when available', () => {
      component.sameAsDeliveryAddress$ = of(false);
      component.isSubmitting$.next(false);
      component['billingAddressForm'] = mockBillingFormWithStatus('INVALID');
      const getBillingAddressFormSpy = spyOn(worldpayBillingAddressFormService, 'getBillingAddressForm');

      component.disableContinueButton().subscribe((result) => {
        expect(result).toBeTrue();
      });

      expect(getBillingAddressFormSpy).not.toHaveBeenCalled();
    });
    ...
    it('should return true if sameAsDeliveryAddress is false and billingAddressForm is invalid', (done) => {
      ...
      spyOn(worldpayBillingAddressFormService, 'getBillingAddressForm').and.returnValue(mockBillingFormWithStatus('INVALID'));
    ...
    it('should return true if isSubmitting is true', () => {
      ...
      component['billingAddressForm'] = undefined;
      spyOn(worldpayBillingAddressFormService, 'getBillingAddressForm').and.returnValue(mockBillingFormWithStatus('VALID'));
