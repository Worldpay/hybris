...
import { FormBuilder, FormControlStatus, UntypedFormGroup } from '@angular/forms';
...
export abstract class WorldpayApmBaseComponent implements OnInit {
   ...
   * @since 2211.43.0
   */
  disableContinueButton(customFormInvalid?: boolean): Observable<boolean> {
    const billingAddressForm: UntypedFormGroup | undefined = this.billingAddressForm ?? this.billingAddressFormService.getBillingAddressForm();
    const billingAddressFormStatus$: Observable<FormControlStatus> = billingAddressForm?.statusChanges?.pipe(
      startWith(billingAddressForm.status as FormControlStatus)
    ) ?? of<FormControlStatus>('INVALID');
    const sameAsDeliveryAddress$: Observable<boolean> = this.sameAsDeliveryAddress$ ?? this.billingAddressFormService.getSameAsDeliveryAddress() ?? of(false);

    return combineLatest([
      billingAddressFormStatus$,
      sameAsDeliveryAddress$,
      this.isSubmitting$,
    ]).pipe(
      map(([formStatus, sameAsDeliveryAddress, isSubmitting]: [FormControlStatus, boolean, boolean]): boolean => {
        if (isSubmitting) return true;

        if (sameAsDeliveryAddress) {
          return !!customFormInvalid;
        }

        return !!customFormInvalid || formStatus === 'INVALID';
      }),
