...
describe('WorldpayApmComponent', () => {
  ...
  let worldpayApmFacade: WorldpayApmService;
  ...
  beforeEach(() => {
    ...
    worldpayApmFacade = TestBed.inject(WorldpayApmService);
    component.apms = worldpayApmFacade.getWorldpayAvailableApms();
    apmSubject.next({
      code: PaymentMethod.Card
    });
  ...
  describe('selectDefaultCardPaymentMethod', () => {
    it('should select default card payment method when no APM is provided', () => {
      spyOn(worldpayApmFacade, 'selectAPM');
      component['selectDefaultCardPaymentMethod'](null);

      expect(worldpayApmFacade.selectAPM).toHaveBeenCalledWith({ code: PaymentMethod.Card });
    });
